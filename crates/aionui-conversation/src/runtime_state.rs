...
use aionui_common::{ApiError, ConversationStatus};
...
impl ConversationRuntimeStateService {
    pub fn try_claim_turn(self: &Arc<Self>, conversation_id: &str) -> Result<TurnClaim, ApiError> {
        let mut state = self.state.lock().map_err(|_| {
            warn!(
                conversation_id,
                "conversation runtime state lock poisoned while claiming turn"
            );
            ApiError::Internal("conversation runtime state lock poisoned".into())
        })?;
