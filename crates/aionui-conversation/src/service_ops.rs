...
impl ConversationService {
    ...
    pub async fn browse_workspace(
        ...
        let row = self
            ...
            .get(conversation_id)
            .await
            .map_err(|e| ApiError::Internal(format!("Failed to load conversation: {e}")))?
        ...
        let canonical_base = base
            .canonicalize()
            .map_err(|e| ApiError::Internal(format!("Failed to resolve workspace path: {e}")))?;
        ...
        while let Ok(Some(entry)) = dir_reader.next_entry().await {
            ...
            let metadata = tokio::fs::metadata(&entry_path)
                .await
                .map_err(|e| ApiError::Internal(format!("Failed to read entry metadata: {e}")))?;
