...
use aionui_common::{AgentKillReason, AgentType, ApiError, PaginatedResult, ProviderWithModel};
...
mod mock_agent {
    ...
    use aionui_common::{AgentKillReason, AgentType, ApiError, Confirmation, ConversationStatus, TimestampMs};
...
async fn d9_ensure_session_rollbacks_when_build_fails() {
    ...
    let failing_factory: AgentFactory = Arc::new(|_opts: BuildTaskOptions| {
        async move { Err(ApiError::Internal("simulated build failure".into())) }.boxed()
    });
