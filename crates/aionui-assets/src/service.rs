...
impl AssetService {
    ...
    pub fn get_logo(&self, asset_path: &str) -> Result<AssetFile, ApiError> {
        let normalized = normalize_logo_path(asset_path)
            .ok_or_else(|| ApiError::Forbidden(format!("Asset path escapes logos root: {asset_path}")))?;

        let file = LogoAssets::get(&normalized).ok_or_else(|| ApiError::NotFound("Logo asset not found".into()))?;
...
fn build_etag(bytes: &[u8]) -> Result<HeaderValue, ApiError> {
    let digest = Sha256::digest(bytes);
    HeaderValue::from_str(&format!("\"{digest:x}\"")).map_err(|error| ApiError::Internal(error.to_string()))
}
