...
Use `threejs-image-generator` when a strong 2D input improves `threejs-3d-generator` output or when the final asset is 2D rather than 3D. In Aion, that skill prefers the built-in `aionui_image_generation` MCP (Seedream) and falls back to Google's Gemini image API only if the MCP tool is unavailable.
