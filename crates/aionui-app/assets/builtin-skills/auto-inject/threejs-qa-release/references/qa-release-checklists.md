...
- Dev, play, or preview server answering at the correct URL. Prefer `launch_game.mjs --no-open` for QA (`LAUNCH_OK`). Whole-game handoff is `launch_game.mjs --deliver` (`GAME_DELIVERED dist=`). Never foreground `npm run play`. Label `localhost` as a local playtest URL.
