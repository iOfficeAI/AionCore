...
1. [`../lark-shared/SKILL.md`](../lark-shared/GUIDE.md) — 认证、权限处理、全局参数（所有操作通用）
...
## 快速决策
- 用户要**复制文档 / 创建文档副本 / 另存为副本**时，切到 [`lark-drive`](../lark-drive/GUIDE.md)，按其中的复制指引使用 `lark-cli drive files copy`；不要用 `docs +fetch` + `docs +create` 重建正文，也不要走 `drive +export` / `drive +import`。
- 先判定任务路径：找文档 / 导入导出走 [`lark-drive`](../lark-drive/GUIDE.md)；只读 / 摘要用 `docs +fetch` 默认 `simple`；明确旧文本 → 新文本直接 `str_replace`；只有 block 链接、评论锚点、插入 / 替换 / 删除 / 移动才局部 fetch `with-ids`；保真改写已有内容才读 `full`
...
| `<vc-transcribe-tab vc-node-id="...">` | `vc-node-id` -> note_id | [`lark-note`](../lark-note/GUIDE.md)：先 `note +detail --note-id <vc-node-id>` |
