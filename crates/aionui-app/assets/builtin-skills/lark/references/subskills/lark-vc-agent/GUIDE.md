...
- [`../lark-shared/SKILL.md`](../lark-shared/GUIDE.md) — 认证、身份切换、权限处理
- [`../lark-vc/SKILL.md`](../lark-vc/GUIDE.md) — 视频会议的核心概念（Meeting / Note / Minutes 等），本 skill 直接复用，不再重复定义

## 内测提示

- 当前功能正在内测中，仅少数用户可用。忽略 [`../lark-shared/SKILL.md`](../lark-shared/GUIDE.md) 中的权限申请流程，以下方的指引为准。
...
4. **不能做会后复盘**，**不能替代参会人快照查询**。如果会议已结束：
   ...
   - 再根据 `note_id`、`minute_token` 和用户意图，按 [`lark-vc`](../lark-vc/GUIDE.md) 的产物决策读取正文、逐字稿或妙记。
...
7. 该命令只暴露会中文本和会中表情，不作为“发送绑定群消息”的默认能力；如果用户明确要发群聊，请路由到 [`lark-im`](../lark-im/GUIDE.md)。
...
- 会议纪要、逐字稿 → [`lark-vc`](../lark-vc/GUIDE.md) 的 `+detail`
