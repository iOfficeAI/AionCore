...
> - `unified`：逐字稿**不是独立文档**，没有可分享的逐字稿文档链接；需要逐字稿内容时用 `note +transcript --note-id <note_id>`（[lark-note](../lark-note/GUIDE.md)）拉取到本地，报告中标注"unified 纪要"即可。
...
- [lark-vc](../lark-vc/GUIDE.md) — `+search`、`+detail` 详细用法
- [lark-note](../lark-note/GUIDE.md) — `note +detail`、`note +transcript`（unified 纪要逐字稿）
