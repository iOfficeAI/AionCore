...
| 拿这条妙记关联的纪要文档（`note_doc_token` / `verbatim_doc_token` / `shared_doc_tokens`） | `+detail` 取顶层 `note_id` → [`note +detail --note-id`](../lark-note/GUIDE.md) |
...
| 用户同时提到"会议/开会"和"妙记" | 先 [lark-vc](../lark-vc/GUIDE.md)（`+search` → `+recording`）获取 `minute_token`，再本 skill |
...
1. 如果是会议的妙记，应优先通过 [lark-vc](../lark-vc/GUIDE.md) 定位会议并获取 `minute_token`。
...
4. 新说话人姓名用 [lark-contact](../lark-contact/GUIDE.md) 解析为 `ou_` open_id。
...
`minutes +detail` 顶层直接返回 `note_id`（仅在该妙记关联纪要时存在）。不需要绕回 [lark-vc](../lark-vc/GUIDE.md)，直接：
...
- 搜索历史会议记录、查参会人快照 → [lark-vc](../lark-vc/GUIDE.md)
- 未来日程 / 日历查询 → [lark-calendar](../lark-calendar/GUIDE.md)
- 已知 `note_id` 直接读纪要详情 → [lark-note](../lark-note/GUIDE.md)
- 飞书任务清单（个人 Todo / 共享清单） → [lark-task](../lark-task/GUIDE.md)
- 只有自然语言纪要标题、没有 `minute_token` / 妙记 URL / 本地音视频时定位逐字稿 → 文档搜索（[lark-drive](../lark-drive/GUIDE.md) / [lark-doc](../lark-doc/GUIDE.md)）
