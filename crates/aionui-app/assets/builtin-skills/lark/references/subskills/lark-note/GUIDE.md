...
身份：仅使用 `--as user`。使用前阅读 [`../lark-shared/SKILL.md`](../lark-shared/GUIDE.md)。
...
- `note_doc_token`：纪要正文文档，正文读取交给 [lark-doc](../lark-doc/GUIDE.md)。
- `verbatim_doc_token`：普通纪要逐字稿文档；unified 逐字稿不按这个 token 路由。

## 不在本 Skill 范围

- 通过 `meeting_id` 定位纪要（`note_id`）→ [lark-vc](../lark-vc/GUIDE.md)（`vc +detail`）。
- 通过 `minute_token` 定位纪要（`note_id`）→ [lark-minutes](../lark-minutes/GUIDE.md)（`minutes +detail` 顶层返回 `note_id`）。
- 通过日程 `event_id` 定位会议(`meeting_id`) / 用户绑定纪要(`meeting_note`) → [lark-calendar](../lark-calendar/GUIDE.md)（`calendar +meeting`）。
- 自然语言纪要标题搜索 → [lark-drive](../lark-drive/GUIDE.md) / [lark-doc](../lark-doc/GUIDE.md)。
- Docx 正文读取 → [lark-doc](../lark-doc/GUIDE.md)。
- 妙记基础信息与媒体文件 → [lark-minutes](../lark-minutes/GUIDE.md)。
