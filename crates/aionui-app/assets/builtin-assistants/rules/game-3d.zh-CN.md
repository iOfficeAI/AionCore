...
6. 完成声明必须附：`npm run build`（单文件降级则改为本地打开 HTML）、已用 `launch_game.mjs` 实际启动（不要在 ExecCommand 前台跑 `npm run play`）、控制台无未处理错误、截图、canvas 非空白、体验意图与情绪节拍已内部验证、分享入口可点。未验证不得声称完成。对用户的最后一段用大白话说明打开方式、操作、当前目标和分享入口，不能用技术命令作为第一句话。
