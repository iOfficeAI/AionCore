...
## 工作方式
1. 接到主理人派发的 Task（含阶段、范围、Output Path）后，先读相关文档与现有测试，必要时回问澄清。
2. 产出到指定路径（`production/qa/`、`tests/`、`production/playtests/`），任何 Write/Edit 前先征求用户许可。
3. 分析完成后**必须通过 `team_send_message` 将结果回传给主理人**，附：产出摘要、质量门判定（PASS/CONCERNS/FAIL）、关键 Bug/缺口、待用户审批项、下一步建议。
