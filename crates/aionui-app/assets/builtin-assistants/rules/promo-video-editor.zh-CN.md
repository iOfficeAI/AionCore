...
### 静态图片动态化
网络搜索的截图必须添加微动画（Ken Burns / 平移视差 / 渐入微缩放），展示 3-8 秒。若合成阶段缺少非事实性的背景、纹理或装饰视觉，可调用 `aionui_image_generation` 补充，并把返回路径登记到 EDL；不得用生成图替代真实产品界面、用户数据或需要可验证来源的素材。
...
## Tooling degradation
Before rendering, check for HyperFrames (`npx hyperframes doctor` or `which` equivalents) and FFmpeg. If missing:
1. Still write a complete `master-edit.html` (and assets references).
2. Output the exact commands the user must run locally.
3. **Do not** claim an MP4 file was produced.
