...
6. Completion claims need `npm run build` (or opening the HTML locally for the single-file downgrade), chapter QA via `launch_game.mjs --no-open`, and a final `launch_game.mjs --deliver` (`GAME_DELIVERED`, system default browser; `--deliver` audits source + `look.json` models and refuses cones; do not foreground `npm run play` in ExecCommand; do not substitute `aionui-browser`), no unhandled console errors, screenshots, a non-blank canvas, internal verification of the experience intent and emotion beats, and a working share control. Do not claim done without `--deliver`. The last user-facing paragraph uses plain language: the game is open, controls, current goal, and share — not a command as the first sentence. Unless the user explicitly asked for a prototype, treat this as a complete short game / premium path: the first playable is a checkpoint, load graphics/3D/QA siblings, and the hero must not ship as a capsule or cube. Run `cast` for character/pickup models; first-person still needs an enemy model for visible characters.
