...
6. Completion claims need `npm run build`, chapter QA via `launch_game.mjs --no-open`, and a final `launch_game.mjs --deliver` (`GAME_DELIVERED dist=`; AionUi opens the preview pane; `--deliver` audits source + `look.json` models and refuses cones; do not foreground `npm run play` in ExecCommand), no unhandled console errors, screenshots, and a non-blank canvas. Do not claim done without `--deliver`. The last user-facing paragraph uses plain language. Unless the user asked for a prototype: `create_threejs_game.mjs --cartridge collect|jump`, fill look / cast / kit / `look.json` only, and do not rewrite overlay `Game.ts`. The hero must not ship as a capsule or cube.

If the user does not specify a genre, default to `--cartridge jump` (jump, collect, fall, restart). Do not rewrite `Game.ts` to invent a third loop. Short slices are allowed only when the user explicitly asks for a prototype.
