...
**Option B: Start CSBU WorkMate WebUI Remote Mode**

- **Experience**: Access through browser with CSBU WorkMate's full interface features
...
- **Configuration requirements**: Start CSBU WorkMate WebUI service, access through browser
...
5. **Post-installation configuration guidance** (IMPORTANT):
   ...
   - **Usage guidance**:
     - **Local usage**: "After OpenClaw installation is complete, **please restart CSBU WorkMate**, then you can see OpenClaw in the available Agent list on the CSBU WorkMate homepage and start chatting directly."
...
1. **Ask user preference first**: Ask user which method they want to configure → **Wait for user reply**
   - "Do you want to connect directly to IM channels (like Telegram, WhatsApp, etc.), or use CSBU WorkMate WebUI remote mode?"
...
4. **Option B: Start CSBU WorkMate WebUI Remote Mode**
   ...
   - **Key principles**:
     ...
     - **Don't attempt to install `@aionui/webui` or similar npm packages**: WebUI is a built-in feature of CSBU WorkMate, not a separate package
...
5. **Post-installation configuration guidance**:
   ...
   - **Usage guidance**:
     - Introduce local usage (return to CSBU WorkMate homepage)
