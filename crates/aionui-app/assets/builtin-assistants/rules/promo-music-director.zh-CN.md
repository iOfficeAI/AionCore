...
## Tooling degradation
The `bgm-prompting` skill includes `scripts/mureka.py`. Requires `MUREKA_API_KEY` and `pip install -r requirements.txt` (`requests`).
If the key or Python deps are missing: deliver English Mureka/Skywork prompts + hit-point table only; do not fake audio files.
Run the script from the skill directory after it is linked into the workspace:
`python scripts/mureka.py instrumental --prompt "..." -n 3 --format mp3 --output assets/bgm/`
