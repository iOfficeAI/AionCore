...
pub(super) async fn fetch_openai_compatible(
    ...
    let body: OpenAiModelsResponse = resp
        .json()
        .await
        .map_err(|e| ApiError::BadGateway(format!("Failed to parse models response: {e}")))?;
...
async fn fetch_anthropic(client: &reqwest::Client, base_url: &str, api_key: &str) -> Result<Vec<ModelInfo>, ApiError> {
    ...
    match result {
        Ok(resp) if resp.status().is_success() => {
            let body: AnthropicModelsResponse = resp
                .json()
                .await
                .map_err(|e| ApiError::BadGateway(format!("Failed to parse Anthropic response: {e}")))?;
            Ok(body.data.into_iter().map(|m| ModelInfo::Id(m.id)).collect())
        }
        Ok(resp) => {
            warn!(
...
async fn fetch_gemini(client: &reqwest::Client, base_url: &str, api_key: &str) -> Result<Vec<ModelInfo>, ApiError> {
    ...
    match result {
        Ok(resp) if resp.status().is_success() => {
            let body: GeminiModelsResponse = resp
                .json()
                .await
                .map_err(|e| ApiError::BadGateway(format!("Failed to parse Gemini response: {e}")))?;
            let models = body
                .models
                .into_iter()
                .map(|m| {
...
async fn fetch_bedrock(config: &FetchConfig) -> Result<Vec<ModelInfo>, ApiError> {
    let bedrock_cfg = config
        .bedrock_config
        .as_ref()
        .ok_or_else(|| ApiError::BadRequest("Bedrock requires bedrockConfig".into()))?;
    ...
    let sdk_config = match bedrock_cfg.auth_method {
        aionui_api_types::BedrockAuthMethod::AccessKey => {
            let key_id = bedrock_cfg
                .access_key_id
                .as_deref()
                .ok_or_else(|| ApiError::BadRequest("accessKeyId is required".into()))?;
            let secret = bedrock_cfg
                .secret_access_key
                .as_deref()
                .ok_or_else(|| ApiError::BadRequest("secretAccessKey is required".into()))?;
    ...
    let resp = client
        ...
        .send()
        .await
        .map_err(|e| ApiError::BadGateway(format!("Bedrock API error: {e}")))?;
...
async fn fetch_new_api(client: &reqwest::Client, base_url: &str, api_key: &str) -> Result<Vec<ModelInfo>, ApiError> {
...
fn check_response_status(resp: &reqwest::Response) -> Result<(), ApiError> {
    if resp.status().is_success() {
        return Ok(());
    }
    Err(ApiError::BadGateway(format!("Remote API returned {}", resp.status())))
}

fn remote_error(e: &reqwest::Error) -> ApiError {
    if e.is_timeout() {
        ApiError::Timeout("Remote API request timed out".into())
    } else {
        ApiError::BadGateway(format!("Remote API request failed: {e}"))
    }
}
