...
use aionui_common::{ApiError, decrypt_string, encrypt_string};
...
impl ProviderService {
    ...
    pub async fn list(&self) -> Result<Vec<ProviderResponse>, ApiError> {
    ...
    pub async fn create(&self, req: CreateProviderRequest) -> Result<ProviderResponse, ApiError> {
    ...
    pub async fn update(&self, id: &str, req: UpdateProviderRequest) -> Result<ProviderResponse, ApiError> {
    ...
    fn row_to_response(&self, row: Provider) -> Result<ProviderResponse, ApiError> {
        let api_key = decrypt_string(&row.api_key_encrypted, &self.encryption_key)?;

        let models: Vec<String> = serde_json::from_str(&row.models)
            .map_err(|e| ApiError::Internal(format!("Failed to parse models JSON: {e}")))?;
        let capabilities = serde_json::from_str(&row.capabilities)
            .map_err(|e| ApiError::Internal(format!("Failed to parse capabilities JSON: {e}")))?;
...
fn serialize_opt<T: serde::Serialize>(val: &Option<T>, field: &str) -> Result<Option<String>, ApiError> {
    val.as_ref()
        .map(serde_json::to_string)
        .transpose()
        .map_err(|e| ApiError::Internal(format!("Failed to serialize {field}: {e}")))
}

/// Serialize a value to JSON string.
fn serialize_json<T: serde::Serialize>(val: &T, field: &str) -> Result<String, ApiError> {
    serde_json::to_string(val).map_err(|e| ApiError::Internal(format!("Failed to serialize {field}: {e}")))
}

/// Deserialize an optional JSON string into a typed value.
pub(crate) fn deserialize_opt<T: DeserializeOwned>(json: &Option<String>, field: &str) -> Result<Option<T>, ApiError> {
    json.as_deref()
        .map(serde_json::from_str)
        .transpose()
        .map_err(|e| ApiError::Internal(format!("Failed to parse {field} JSON: {e}")))
}
...
fn validate_create_request(req: &CreateProviderRequest) -> Result<(), ApiError> {
...
fn validate_id(id: &str) -> Result<(), ApiError> {
    ...
    if trimmed.len() > 128 {
        return Err(ApiError::BadRequest("id must be at most 128 characters".into()));
    }
    if !trimmed
        .chars()
...
fn validate_update_request(req: &UpdateProviderRequest) -> Result<(), ApiError> {
    if let Some(ref platform) = req.platform
        && platform.trim().is_empty()
    {
        return Err(ApiError::BadRequest("platform cannot be empty".into()));
    }
