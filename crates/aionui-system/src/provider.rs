...
use aionui_db::{
    CreateProviderParams, IProviderRepository, IResourceShareRepository, ResourceAccess, ShareResourceType,
...
use crate::provider_network::{validate_member_provider_endpoint, validate_member_provider_runtime};
...
impl ProviderService {
    ...
    pub async fn list(&self, user_id: &str) -> Result<Vec<ProviderResponse>, SystemError> {
        ...
        for row in rows {
            out.push(self.row_to_response_for_user(user_id, row).await?);
        }
        Ok(out)
    }
    ...
    pub async fn create(
        ...
        validate_create_network_access(&req, network_policy).await?;
        ...
        self.row_to_response_for_user(user_id, row).await
    }
    ...
    pub async fn update(
        ...
        let existing = if network_policy == OutboundNetworkPolicy::PublicOnly {
            Some(
                self.repo
        ...
        validate_update_network_access(&req, network_policy, existing.as_ref()).await?;
    ...
    async fn row_to_response_for_user(&self, user_id: &str, row: Provider) -> Result<ProviderResponse, SystemError> {
        let mut response = self.row_to_response(row)?;
        if self.should_mask_secrets(user_id, &response.id).await? {
            // View-only grantees never receive raw secrets.
            response.api_key.clear();
            if let Some(ref mut bedrock) = response.bedrock_config {
    ...
    async fn should_mask_secrets(&self, user_id: &str, provider_id: &str) -> Result<bool, SystemError> {
        let Some(share_repo) = &self.share_repo else {
            return Ok(false);
        };
        let access = share_repo
            .resolve_access(ShareResourceType::Provider, provider_id, user_id)
            .await?;
        // Owner and edit grantees may see secrets; view-only must not.
        Ok(access == ResourceAccess::View)
    }
...
async fn validate_create_network_access(
    req: &CreateProviderRequest,
    network_policy: OutboundNetworkPolicy,
) -> Result<(), SystemError> {
    validate_bedrock_profile_access(req.bedrock_config.as_ref(), network_policy)?;
    if network_policy == OutboundNetworkPolicy::PublicOnly {
        return validate_member_provider_runtime(&req.platform, &req.base_url).await;
    }
    if !req.base_url.trim().is_empty() {
        network_guard::validate_url(network_policy, &req.base_url).await?;
    }
    Ok(())
}

async fn validate_update_network_access(
    req: &UpdateProviderRequest,
    network_policy: OutboundNetworkPolicy,
    ...
    validate_bedrock_profile_access(req.bedrock_config.as_ref(), network_policy)?;
    if network_policy == OutboundNetworkPolicy::PublicOnly {
        let existing = existing.ok_or_else(|| SystemError::Internal("Missing provider network context".into()))?;
        let platform = req.platform.as_deref().unwrap_or(&existing.platform);
        let base_url = req.base_url.as_deref().unwrap_or(&existing.base_url);
        validate_member_provider_endpoint(platform, base_url)?;

        if req.platform.is_some() || req.base_url.is_some() {
            return validate_member_provider_runtime(platform, base_url).await;
        }
        return Ok(());
    }
    ...
    {
        network_guard::validate_url(network_policy, base_url).await?;
    }
    Ok(())
}

fn validate_bedrock_profile_access(
    config: Option<&aionui_api_types::BedrockConfig>,
    network_policy: OutboundNetworkPolicy,
) -> Result<(), SystemError> {
    if network_policy == OutboundNetworkPolicy::PublicOnly
        && config.is_some_and(|config| matches!(config.auth_method, aionui_api_types::BedrockAuthMethod::Profile))
    {
        return Err(SystemError::BadRequest(
            "Bedrock host profiles are available only to site administrators".into(),
        ));
    }
    Ok(())
}
...
mod tests {
    ...
    const TEST_NETWORK_POLICY: OutboundNetworkPolicy = OutboundNetworkPolicy::Unrestricted;
    ...
    async fn create_and_list() {
        let svc = setup().await;
        let created = svc
            .create(TEST_USER_ID, sample_create_request(), TEST_NETWORK_POLICY)
            .await
            .unwrap();
    ...
    async fn create_with_provided_id() {
        ...
        let created = svc.create(TEST_USER_ID, req, TEST_NETWORK_POLICY).await.unwrap();
    ...
    async fn create_with_duplicate_id_returns_conflict() {
        ...
        svc.create(TEST_USER_ID, req1, TEST_NETWORK_POLICY).await.unwrap();
