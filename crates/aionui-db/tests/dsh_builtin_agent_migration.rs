...
async fn dsh_builtin_acp_metadata_is_seeded() {
    ...
    let dsh = repo.get("d5e89a01").await.unwrap().expect("seeded DSH ACP row");
    ...
    assert_eq!(dsh.icon.as_deref(), Some("/api/assets/logos/ai-major/deepseek.svg"));
    assert_eq!(dsh.agent_source_info.as_deref(), Some(r#"{"binary_name":"dsh"}"#));
    assert_eq!(dsh.native_skills_dirs.as_deref(), Some(r#"[".agents/skills"]"#));
