...
type MemoryPolicyTuple = (Option<bool>, Option<bool>, Option<i64>, i64);
...
impl SqliteMemoryRepository {
    ...
    async fn effective_policy_on(
        connection: &mut SqliteConnection,
        user_id: &str,
        conversation_id: &str,
    ) -> Result<EffectiveMemoryPolicyRow, DbError> {
        Self::ensure_conversation_on(connection, user_id, conversation_id).await?;
        sqlx::query("INSERT INTO memory_settings (user_id,updated_at) VALUES (?,0) ON CONFLICT(user_id) DO NOTHING")
            .bind(user_id)
            .execute(&mut *connection)
            .await?;
        let settings: MemorySettingsRow = sqlx::query_as("SELECT * FROM memory_settings WHERE user_id = ?")
            .bind(user_id)
            .fetch_one(&mut *connection)
            .await?;
        let policy: Option<MemoryPolicyTuple> = sqlx::query_as(
        ...
        if let Some(conversation_id) = memory_summary_conversation_id(selection_id) {
            ...
            return Ok(sqlx::query_as::<_, ConversationMemoryRow>(
                "SELECT * FROM conversation_memories
                 WHERE user_id = ? AND conversation_id = ? AND (? IS NULL OR updated_at >= ?)",
            )
            .bind(user_id)
            .bind(conversation_id)
            .bind(reset_at)
            ...
            .fetch_optional(&mut *connection)
            .await?
            .map(MemoryRetrievalItemRow::ConversationSummary));
        }
        let row = sqlx::query_as::<_, MemoryEntryDbRow>("SELECT * FROM memory_entries WHERE id = ? AND user_id = ?")
            .bind(selection_id)
            .bind(user_id)
            .fetch_optional(&mut *connection)
            .await?;
...
impl IMemoryRepository for SqliteMemoryRepository {
        ...
        let rows = sqlx::query_as::<_, MemoryEntryDbRow>(
            "SELECT * FROM memory_entries
             WHERE user_id = ? AND state = 'active'
               ...
               AND EXISTS (
                    SELECT 1 FROM memory_sources retrieval_source
                    WHERE retrieval_source.memory_entry_id = memory_entries.id
                      AND (? IS NULL OR retrieval_source.last_observed_at >= ?)
                      AND (? IS NULL OR retrieval_source.conversation_id <> ?)
               )
             ORDER BY
               CASE
    ...
    async fn create_retrieval_snapshot(
        ...
        sqlx::query("BEGIN IMMEDIATE").execute(&mut *connection).await?;
        let result = async {
            let conversation_updated_at: i64 =
                sqlx::query_scalar("SELECT updated_at FROM conversations WHERE id = ? AND user_id = ?")
                    .bind(&input.retrieval.conversation_id)
                    .bind(&input.retrieval.user_id)
                    .fetch_optional(&mut *connection)
                    .await?
                    .ok_or_else(|| DbError::NotFound("Memory retrieval conversation not found".into()))?;
            let policy = Self::effective_policy_on(
                &mut connection,
                &input.retrieval.user_id,
                &input.retrieval.conversation_id,
            )
            .await?;
            if conversation_updated_at != input.expected_conversation_updated_at || policy != input.expected_policy {
                return Err(DbError::Conflict("Memory retrieval snapshot changed".into()));
            }
            let selected_ids: Vec<String> = serde_json::from_str(&input.retrieval.selected_ids_json)
                .map_err(|_| DbError::Conflict("Invalid Memory retrieval selections".into()))?;
            ...
            .bind(&input.retrieval.user_id)
            .bind(&input.retrieval.conversation_id)
            .bind(&input.retrieval.prompt_hash)
            .bind(&input.retrieval.retrieval_version)
            .execute(&mut *connection)
            .await?;
            sqlx::query(
                "INSERT INTO memory_retrievals
                    (id,user_id,conversation_id,prompt_hash,selected_ids_json,estimated_tokens,budget_tokens,
                     retrieval_version,created_at,expires_at) VALUES (?,?,?,?,?,?,?,?,?,?)",
            )
            .bind(&input.retrieval.id)
            .bind(&input.retrieval.user_id)
            .bind(&input.retrieval.conversation_id)
            .bind(&input.retrieval.prompt_hash)
            .bind(&input.retrieval.selected_ids_json)
            .bind(input.retrieval.estimated_tokens)
            .bind(input.retrieval.budget_tokens)
            .bind(&input.retrieval.retrieval_version)
            .bind(input.retrieval.created_at)
    ...
    async fn consume_retrieval_snapshot(
        ...
        let result = async {
            ...
            let retrieval: MemoryRetrievalRow =
                sqlx::query_as("SELECT * FROM memory_retrievals WHERE id = ? AND user_id = ?")
                    .bind(&input.retrieval_id)
                    .bind(&input.user_id)
                    .fetch_optional(&mut *connection)
                    .await?
                    .ok_or_else(|| DbError::NotFound("Memory retrieval not found".into()))?;
            if retrieval.conversation_id != input.conversation_id
                || retrieval.prompt_hash != input.prompt_hash
                || retrieval.retrieval_version != input.retrieval_version
                || retrieval.budget_tokens != input.expected_budget_tokens
                || retrieval.expires_at <= input.now
            {
                return Err(DbError::Conflict("Memory retrieval no longer matches".into()));
            }
            let policy = Self::effective_policy_on(&mut connection, &input.user_id, &input.conversation_id).await?;
            if !policy.enabled || !policy.recall_enabled {
                return Err(DbError::Conflict("Memory recall policy changed".into()));
            }
            ...
            let conversation = sqlx::query_as("SELECT * FROM conversations WHERE id = ? AND user_id = ?")
                ...
                .await?;
            sqlx::query("COMMIT").execute(&mut *connection).await?;
            Ok::<_, DbError>(MemoryRetrievalSnapshotRow {
                retrieval,
                policy,
                conversation,
                items,
...
mod tests {
    ...
    use crate::repository::memory::{
        ...
        memory_entry_content_hash, memory_summary_selection_id,
    };
    ...
    async fn sqlite_memory_candidate_window_cannot_crowd_out_exact_project_workspace() {
        ...
        sqlx::query(
            "INSERT INTO memory_sources
                (memory_entry_id,conversation_id,turn_id,message_ids_json,first_observed_at,last_observed_at)
             SELECT id,'conv_a','turn-' || id,'[]',updated_at,updated_at FROM memory_entries WHERE user_id = ?",
        )
        .bind(USER_A)
        .execute(db.pool())
        .await
    ...
    #[tokio::test]
    async fn sqlite_memory_candidate_window_filters_current_only_and_pre_reset_sources_before_limit() {
        let (repo, _, db) = setup().await;
        for index in 0..200 {
            sqlx::query(
                "INSERT INTO memory_entries
                    (id,user_id,project_id,workspace_key,kind,stable_key,fingerprint,content,state,pinned,user_edited,
                     revision,schema_version,created_at,updated_at)
                 VALUES (?,?,'project-1','workspace-1','issue',?,?,'needle','active',1,0,1,1,?,?)",
            )
            .bind(format!("ineligible-{index:03}"))
            .bind(USER_A)
            .bind(format!("ineligible-key-{index:03}"))
            .bind(format!("ineligible-fingerprint-{index:03}"))
            .bind(3_000 + index)
            .bind(3_000 + index)
            .execute(db.pool())
            .await
            .unwrap();
            let (conversation_id, observed_at) = if index < 100 {
                ("conv_a", 2_000 + index)
            } else {
                ("conv_a2", 100 + index)
            };
            ...
            .bind(format!("ineligible-{index:03}"))
            .bind(conversation_id)
            .bind(format!("ineligible-turn-{index:03}"))
            .bind(observed_at)
            .bind(observed_at)
            .execute(db.pool())
            .await
        ...
        sqlx::query(
            "INSERT INTO memory_entries
                (id,user_id,project_id,workspace_key,kind,stable_key,fingerprint,content,state,pinned,user_edited,
                 revision,schema_version,created_at,updated_at)
             VALUES ('eligible-beyond-window',?,'project-1','workspace-1','issue','eligible-key',
                     'eligible-fingerprint','needle','active',0,0,1,1,1,1)",
        )
        .bind(USER_A)
        .execute(db.pool())
        .await
        ...
        sqlx::query(
            "INSERT INTO memory_sources
                (memory_entry_id,conversation_id,turn_id,message_ids_json,first_observed_at,last_observed_at)
             VALUES ('eligible-beyond-window','conv_a2','eligible-turn','[]',2000,2000)",
        )
        .execute(db.pool())
        .await
        .unwrap();
        ...
        let candidates = repo
            ...
            .unwrap();
        assert_eq!(
            candidates.iter().map(|entry| entry.id.as_str()).collect::<Vec<_>>(),
            ["eligible-beyond-window"],
        );
        assert_eq!(candidates[0].sources[0].conversation_id, "conv_a2");
    }
    ...
    #[tokio::test]
    async fn sqlite_memory_summary_window_filters_ineligible_rows_before_limit_and_snapshot_lookup() {
        let (repo, conversations, db) = setup().await;
        sqlx::query(
            "INSERT INTO conversation_memory_policies
                (user_id,conversation_id,reset_at,lifecycle_epoch,updated_at) VALUES (?,'conv_a',1000,1,1000)",
        )
        .bind(USER_A)
        .execute(db.pool())
        .await
        ...
        sqlx::query(
            "INSERT INTO conversation_memories
                (user_id,conversation_id,project_id,workspace_key,summary_json,through_turn_id,revision,source,
                 schema_version,created_at,updated_at)
             VALUES (?,'conv_a','project-1','workspace-1','{}','turn',1,'memory_update',1,1500,2000)",
        )
        .bind(USER_A)
        .execute(db.pool())
        .await
        .unwrap();
        for index in 0..8 {
            let id = format!("stale-summary-{index}");
            conversations.create(&conversation(&id, USER_A)).await.unwrap();
            sqlx::query(
                "INSERT INTO conversation_memories
                    (user_id,conversation_id,project_id,workspace_key,summary_json,through_turn_id,revision,source,
                     schema_version,created_at,updated_at)
                 VALUES (?,?,'project-1','workspace-1','{}','turn',1,'memory_update',1,100,500)",
            )
            .bind(USER_A)
            .bind(&id)
            .execute(db.pool())
        ...
        let query = MemoryCandidateQueryRow {
            user_id: USER_A.into(),
            project_id: Some("project-1".into()),
            workspace_key: Some("workspace-1".into()),
            current_conversation_id: Some("conv_a".into()),
        ...
        let summaries = repo.retrieval_summaries(query).await.unwrap();
        assert_eq!(
            summaries
                .iter()
        ...
        let stale = sqlx::query_as("SELECT * FROM conversation_memories WHERE conversation_id = 'stale-summary-0'")
            .fetch_one(db.pool())
            .await
            .unwrap();
        let retrieval = |id: &str, selected_id: String| MemoryRetrievalRow {
            id: id.into(),
            user_id: USER_A.into(),
            conversation_id: "conv_a".into(),
            prompt_hash: "prompt".into(),
            selected_ids_json: serde_json::to_string(&[selected_id]).unwrap(),
            estimated_tokens: 10,
            budget_tokens: 2_000,
            retrieval_version: "memory-retrieval-v1".into(),
            created_at: 2_001,
        ...
        assert!(matches!(
            repo.create_retrieval_snapshot(CreateMemoryRetrievalSnapshotRow {
                retrieval: retrieval(
                    "stale-summary-retrieval",
                    memory_summary_selection_id("stale-summary-0")
                ),
                expected_policy: policy.clone(),
                expected_conversation_updated_at: conversation_updated_at,
                items: vec![MemoryRetrievalItemRow::ConversationSummary(stale)],
            })
            .await,
            Err(DbError::Conflict(_))
        ));
        let current = sqlx::query_as("SELECT * FROM conversation_memories WHERE conversation_id = 'conv_a'")
            .fetch_one(db.pool())
            .await
            .unwrap();
        assert!(matches!(
            repo.create_retrieval_snapshot(CreateMemoryRetrievalSnapshotRow {
                retrieval: retrieval("current-summary-retrieval", memory_summary_selection_id("conv_a")),
                expected_policy: policy.clone(),
                expected_conversation_updated_at: conversation_updated_at,
                items: vec![MemoryRetrievalItemRow::ConversationSummary(current)],
            })
            .await,
            Err(DbError::Conflict(_))
        ));

        let eligible = summaries[0].clone();
        let eligible_id = memory_summary_selection_id(&eligible.conversation_id);
        let row = retrieval("eligible-summary-retrieval", eligible_id.clone());
        repo.create_retrieval_snapshot(CreateMemoryRetrievalSnapshotRow {
            retrieval: row.clone(),
            expected_policy: policy,
            expected_conversation_updated_at: conversation_updated_at,
            items: vec![MemoryRetrievalItemRow::ConversationSummary(eligible.clone())],
        })
        .await
        .unwrap();
        let snapshot = repo
            .consume_retrieval_snapshot(ConsumeMemoryRetrievalSnapshotRow {
                ...
                expected_budget_tokens: row.budget_tokens,
                now: 2_002,
            })
            .await
            .unwrap();
        assert_eq!(
            snapshot.items,
            vec![MemoryRetrievalItemRow::ConversationSummary(eligible)]
        );
    }
    ...
    #[tokio::test]
    async fn sqlite_memory_retrieval_snapshot_fences_candidate_policy_and_replacement_races() {
        let (repo, conversations, db) = setup().await;
        claimed_job(&repo, "job-retrieval-snapshot", "conv_a", "turn-1").await;
        repo.commit_update(commit(
            "job-retrieval-snapshot",
            "conv_a",
            "turn-1",
            0,
            vec![entry("snapshot-entry", "fp-snapshot", vec![source("conv_a", "turn-1")])],
            20,
        ))
        .await
        .unwrap();
        let expected = repo.get_entry(USER_A, "snapshot-entry").await.unwrap().unwrap();
        let policy = repo.effective_policy(USER_A, "conv_a2").await.unwrap();
        let conversation_updated_at = conversations.get("conv_a2").await.unwrap().unwrap().updated_at;
        let retrieval = MemoryRetrievalRow {
            id: "snapshot-retrieval".into(),
            user_id: USER_A.into(),
            conversation_id: "conv_a2".into(),
            prompt_hash: "prompt".into(),
            selected_ids_json: r#"["snapshot-entry"]"#.into(),
            estimated_tokens: 10,
            budget_tokens: 2_000,
            retrieval_version: "memory-retrieval-v1".into(),
            created_at: 30,
        ...
        sqlx::query(
            "UPDATE memory_entries SET content = 'changed',revision = revision + 1 WHERE id = 'snapshot-entry'",
        )
        .execute(db.pool())
        .await
        .unwrap();
        assert!(matches!(
            repo.create_retrieval_snapshot(CreateMemoryRetrievalSnapshotRow {
                ...
                expected_policy: policy.clone(),
                expected_conversation_updated_at: conversation_updated_at,
                items: vec![MemoryRetrievalItemRow::Entry(expected)],
            })
            .await,
            Err(DbError::Conflict(_))
        ));

        let current = repo.get_entry(USER_A, "snapshot-entry").await.unwrap().unwrap();
        repo.create_retrieval_snapshot(CreateMemoryRetrievalSnapshotRow {
            retrieval: retrieval.clone(),
            expected_policy: policy,
            expected_conversation_updated_at: conversation_updated_at,
            items: vec![MemoryRetrievalItemRow::Entry(current)],
        })
        .await
        .unwrap();
        repo.update_conversation_policy(UpdateConversationMemoryPolicyRow {
