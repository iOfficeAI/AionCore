...
CREATE TABLE IF NOT EXISTS memory_change_sets (
    ...
    added_ids_json      TEXT NOT NULL CHECK(json_valid(added_ids_json)),
    refined_ids_json    TEXT NOT NULL CHECK(json_valid(refined_ids_json)),
    superseded_ids_json TEXT NOT NULL CHECK(json_valid(superseded_ids_json)),
    conflict_ids_json   TEXT NOT NULL CHECK(json_valid(conflict_ids_json)),
    created_at          INTEGER NOT NULL,
    FOREIGN KEY (user_id) REFERENCES users(id) ON DELETE CASCADE,
    FOREIGN KEY (conversation_id) REFERENCES conversations(id) ON DELETE CASCADE
);
