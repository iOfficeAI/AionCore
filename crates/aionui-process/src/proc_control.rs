...
mod tests {
    ...
    // Two distinct, stable epochs for the identity gate.
    fn run_a() -> Uuid {
        Uuid::from_u128(0xA)
    }
    fn run_b() -> Uuid {
    ...
    /// classify_liveness is the KILL-SAFETY core (3-defense identity gate). A
    /// table over every decision branch — a kill is authorized ONLY on
    /// `Liveness::Match`, so every other row must NOT be Match. This crate had
    /// ZERO tests for it (PROC-G1); the logic was correct-but-unverified.
    /// (name, recorded_start, observed, recorded_epoch, current_epoch, expect).
    type Case = (&'static str, Option<u64>, ObservedLiveness, Uuid, Uuid, Liveness);

    #[test]
    fn classify_liveness_decision_table() {
        let cases: &[Case] = &[
            // (name, recorded_start, observed, recorded_epoch, current_epoch, expect)
            (
                "prior-run orphan, start-time matches → Match (the ONLY reapable)",
                Some(111),
                ObservedLiveness::Alive { start_ticks: Some(111) },
                run_a(),
                run_b(),
                Liveness::Match,
            ),
            (
                "our own current run (epoch equal) → DiffEpoch, never reap",
                Some(111),
                ObservedLiveness::Alive { start_ticks: Some(111) },
                run_a(),
                run_a(),
            ...
            (
                "same pid, different start-time → RecycledPid, never reap",
                Some(111),
                ObservedLiveness::Alive { start_ticks: Some(222) },
                run_a(),
                run_b(),
                Liveness::RecycledPid,
            ),
            (
                "process gone (ESRCH) → Gone",
                Some(111),
                ObservedLiveness::Gone,
                run_a(),
                run_b(),
            ...
            (
                "EPERM (alive but unprovable) → EpermAlive, never reap",
                Some(111),
                ObservedLiveness::EpermAlive,
                run_a(),
                run_b(),
                Liveness::EpermAlive,
            ),
            (
                "could not record start-time → Unknown, never reap",
                None,
                ObservedLiveness::Alive { start_ticks: Some(111) },
                run_a(),
                run_b(),
            ...
            (
                "could not observe start-time → Unknown, never reap",
                Some(111),
                ObservedLiveness::Alive { start_ticks: None },
                run_a(),
                run_b(),
            ...
            (
                "F57: recorded start-time 0 (corrupt sentinel) → Unknown, refuse to gate on garbage",
                Some(0),
                ObservedLiveness::Alive { start_ticks: Some(0) },
                run_a(),
                run_b(),
                Liveness::Unknown,
            ),
            (
                "F57: observed start-time 0 → Unknown even if recorded matches non-zero",
                Some(111),
                ObservedLiveness::Alive { start_ticks: Some(0) },
                run_a(),
                run_b(),
        ...
        for (name, rec, obs, re, ce, expect) in cases {
            let got = classify_liveness(*rec, *obs, *re, *ce);
            assert_eq!(got, *expect, "case: {name}");
        }
    }

    /// The ONLY value that authorizes a kill is Match — assert no other input
    /// shape produces it (the negative-space guard the kill path relies on).
    #[test]
    fn only_prior_run_matching_start_time_is_reapable() {
        // recycled pid must never be reapable even across epochs
        assert_ne!(
            classify_liveness(
                Some(5),
                ObservedLiveness::Alive { start_ticks: Some(9) },
                run_a(),
                run_b()
            ),
            Liveness::Match,
            "recycled PID must not be Match"
        );
        // zero sentinel must never be reapable
        assert_ne!(
            classify_liveness(
                Some(0),
                ObservedLiveness::Alive { start_ticks: Some(0) },
            ...
            Liveness::Match,
            "zero sentinel must not be Match"
        );
    }

    /// F50 / IC-6: group-kill target guards. pgid<=1 (blast-floor) and pgid that
    /// would wrap a negative i32 must return None (never collapse a group-kill
    /// into a stray single-pid signal or kill pgid 0/-1 = "every process").
    #[test]
    fn group_kill_target_blast_floor_and_overflow() {
        assert_eq!(group_kill_target(0), None, "pgid 0 = whole-world, refuse");
        assert_eq!(group_kill_target(1), None, "pgid 1 = init group, refuse (IC-6 floor)");
        assert_eq!(group_kill_target(2), Some(-2), "normal pgid → negative group target");
        assert_eq!(group_kill_target(12345), Some(-12345));
        // > i32::MAX would wrap; must refuse (F50)
        assert_eq!(
            group_kill_target(u32::MAX),
            None,
            "pgid > i32::MAX must not wrap to a positive single-pid kill"
        );
        assert_eq!(group_kill_target(0x8000_0000), None, "exactly i32::MAX+1 → refuse");
    }

    /// F50: pid signal target must be a positive i32 or None (never wrap).
    #[test]
    fn pid_signal_target_positive_or_none() {
        assert_eq!(pid_signal_target(0), None, "pid 0 has special signal semantics; refuse");
        assert_eq!(pid_signal_target(1), Some(1));
        assert_eq!(pid_signal_target(99999), Some(99999));
        assert_eq!(
            pid_signal_target(u32::MAX),
            None,
            "pid > i32::MAX must not wrap negative"
        );
        assert_eq!(pid_signal_target(0x8000_0000), None);
    }

    /// LIVENESS-C7: `current_process_group` (the reaper's-own-pgid source for the
    /// self-group kill guard) is crate-internal, so it is tested here, not from
    /// `tests/`. On Unix it must equal `getpgrp()` and be a sane (>1) value; on
    /// non-Unix it is `None`.
    #[test]
    fn current_process_group_matches_syscall_on_unix() {
        #[cfg(unix)]
        {
            let got = current_process_group().expect("Unix process always has a pgid");
            let expected = u32::try_from(unsafe { libc::getpgrp() }).unwrap();
            assert_eq!(got, expected, "must reflect the live getpgrp()");
            assert!(got > 1, "a real pgid is never the 0/1 kernel sentinels");
        }
        #[cfg(not(unix))]
        assert_eq!(current_process_group(), None, "no pgid concept off-Unix");
    }

    /// LIVENESS-C6: on platforms with a start-time accessor, reading our OWN
    /// process's start-time yields a stable, non-zero token (the identity-gate
    /// value). On macOS the token packs `(secs << 20) | usecs`; assert the
    /// unpacked fields are a plausible wall-clock instant. Stability across two
    /// reads is the load-bearing property (the gate compares recorded vs live).
    #[test]
    fn read_process_start_time_is_stable_and_well_formed() {
        let me = std::process::id();
        let first = read_process_start_time(me);
        #[cfg(any(target_os = "linux", target_os = "macos"))]
        {
            let t = first.expect("Linux/macOS expose a start-time for self");
            assert!(t > 0, "start-time token is non-zero (zero is the F57 corrupt sentinel)");
            assert_eq!(
                read_process_start_time(me),
                Some(t),
                "stable across reads (gate relies on this)"
            );
            #[cfg(target_os = "macos")]
            {
                // macOS packing: secs in the high bits, usecs (< 1e6) in low 20.
                let usecs = t & 0xF_FFFF;
                let secs = t >> 20;
                assert!(usecs < 1_000_000, "unpacked usecs field is a valid microsecond count");
                assert!(secs > 1_000_000_000, "unpacked secs is a plausible UNIX timestamp");
            }
        }
        #[cfg(not(any(target_os = "linux", target_os = "macos")))]
        let _ = first; // other platforms may legitimately return None
    }
}
