...
#[cfg(test)]
mod tests {
    //! Tests for the pure / cheap helpers in this module: the workspace-cwd
    //! validation guard (input validation, platform-independent) and the
    //! process-group-id derivation. The real `ManagedProcess` lifecycle
    //! (spawn/kill/Drop/take_stdio) needs a live child and is covered by
    //! real-process integration tests (see audit PROC-L*).

    use super::*;

    /// INPUTVAL-B1: any whitespace in ANY path segment is rejected with the
    /// dedicated `WorkspacePathContainsWhitespaceRuntimeUnsupported` variant
    /// (the bundled runtime cannot handle it — this was the F1 spawn-fail
    /// incident root cause, and was previously untested).
    #[test]
    fn prepare_command_cwd_rejects_whitespace_in_any_segment() {
        for bad in ["/tmp/my project", "/tmp/proj/sub folder/bin", "/tmp/dir\twith\ttab"] {
            let err = prepare_command_cwd(bad).expect_err("whitespace cwd must be rejected");
            assert!(
                matches!(err, ProcessError::WorkspacePathContainsWhitespaceRuntimeUnsupported(_)),
                "{bad:?} → expected WhitespaceRuntimeUnsupported, got {err:?}"
            );
        }
    }

    /// INPUTVAL-B1 boundary: the whitespace guard fires BEFORE the filesystem
    /// metadata check, so a non-existent whitespace path still yields the
    /// whitespace error (not a "does not exist" error). This pins the ordering
    /// that makes the guard testable without a real directory.
    #[test]
    fn prepare_command_cwd_whitespace_guard_precedes_existence_check() {
        let err = prepare_command_cwd("/nonexistent path/that does not exist").expect_err("must reject");
        assert!(
            matches!(err, ProcessError::WorkspacePathContainsWhitespaceRuntimeUnsupported(_)),
            "whitespace must be detected before the metadata/existence check, got {err:?}"
        );
    }

    /// INPUTVAL-B2: an empty or whitespace-only cwd is a `BadRequest`
    /// ("workspace directory is empty"), distinct from the whitespace-segment
    /// variant.
    #[test]
    fn prepare_command_cwd_rejects_empty_or_blank() {
        for blank in ["", "   ", "\t", "\n"] {
            let err = prepare_command_cwd(blank).expect_err("blank cwd must be rejected");
            assert!(
    ...
    /// INPUTVAL-B1 negative: a clean, existing directory with no whitespace
    /// passes the guard and returns the canonical PathBuf. Uses the OS temp dir,
    /// which is guaranteed to exist; skip if its path happens to contain
    /// whitespace (some CI home dirs do) so the assertion stays meaningful.
    #[test]
    fn prepare_command_cwd_accepts_clean_existing_dir() {
        let dir = std::env::temp_dir();
        let has_ws = dir
            .components()
            .any(|c| c.as_os_str().to_string_lossy().contains(char::is_whitespace));
        if has_ws {
            return; // temp dir itself has whitespace; guard would (correctly) reject.
        }
        let out = prepare_command_cwd(&dir.to_string_lossy()).expect("clean existing dir accepted");
        assert_eq!(out, dir, "returns the validated path unchanged");
    }

    /// PROC-L16: on Unix the child is spawned as its own process-group leader
    /// (Builder calls `process_group(0)`), so the tracked pgid equals the pid;
    /// on non-Unix there is no pgid concept and it is `None`.
    #[test]
    fn tracked_process_group_id_is_pid_on_unix_none_elsewhere() {
        #[cfg(unix)]
        assert_eq!(tracked_process_group_id(12345), Some(12345), "Unix leader pgid == pid");
        #[cfg(not(unix))]
        assert_eq!(tracked_process_group_id(12345), None, "no pgid concept off-Unix");
    }
}
