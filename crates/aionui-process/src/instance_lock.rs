...
#[cfg(test)]
mod tests {
    //! Single-process-safe lock tests (fresh acquire, epoch mint, path accessor,
    //! LockHeld shape). True two-instance MUTUAL EXCLUSION (LOCK-12) needs a
    //! second OS process — `fs2` advisory locks are per-open-file-description and
    //! a single test process can re-lock its own fd — so that contention test is
    //! a concurrency-harness gap (see audit), not asserted here.

    use tempfile::TempDir;

    use super::*;

    /// LOCK-1/5/6: a fresh acquire on an empty data dir succeeds, creates the
    /// lock file at the namespaced path, and mints a per-run epoch.
    #[test]
    fn fresh_acquire_succeeds_creates_file_and_mints_epoch() {
        let tmp = TempDir::new().unwrap();
        let (lock, epoch) = acquire_instance_lock(tmp.path()).expect("fresh acquire succeeds");

        assert_eq!(
            lock.path(),
            tmp.path().join(SUBDIR).join(LOCK_FILE),
            "lock at namespaced path"
        );
        assert!(lock.path().exists(), "lock file created on disk");
        assert_ne!(epoch, Uuid::nil(), "a real v4 epoch is minted");
        assert_eq!(epoch.get_version_num(), 4, "epoch is a v4 (random) UUID");
    }

    /// LOCK-5: each successful acquisition mints a DISTINCT epoch (acquire →
    /// drop → re-acquire yields a different UUID; epochs are never reused).
    #[test]
    fn each_acquire_mints_a_distinct_epoch() {
        let tmp = TempDir::new().unwrap();
        let (lock1, e1) = acquire_instance_lock(tmp.path()).expect("first acquire");
        drop(lock1); // release so the successor can take it
        let (_lock2, e2) = acquire_instance_lock(tmp.path()).expect("re-acquire after drop");
        assert_ne!(e1, e2, "a fresh epoch is minted on every acquisition");
    }

    /// LOCK-8: `LockHeld` carries the contended path and renders it in `Display`
    /// (callers log/inspect which lock blocked them).
    #[test]
    fn lock_held_carries_and_displays_the_path() {
        let held = LockHeld {
            path: PathBuf::from("/data/runtime/aionui-process/instance.lock"),
        };
        assert_eq!(held.path, PathBuf::from("/data/runtime/aionui-process/instance.lock"));
        assert!(
            held.to_string().contains("instance.lock"),
            "Display must surface the contended path, got {held}"
        );
    }

    /// LOCK-4: `create_dir_all` is idempotent — acquiring, dropping, and
    /// re-acquiring in the same dir does not error on the pre-existing dir/file.
    #[test]
    fn reacquire_in_existing_dir_is_benign() {
        let tmp = TempDir::new().unwrap();
        let (lock1, _) = acquire_instance_lock(tmp.path()).expect("first");
        drop(lock1);
        acquire_instance_lock(tmp.path()).expect("re-acquire over existing dir/file must not error");
    }
}
