...
type PendingSender = oneshot::Sender<Result<Value, ApiError>>;
...
impl OpenClawConnection {
    ...
    ) -> Result<(Arc<Self>, HelloOk), ApiError> {
        let (ws_stream, _) = tokio_tungstenite::connect_async(url)
            .await
            .map_err(|e| ApiError::Internal(format!("OpenClaw WebSocket connection failed: {e}")))?;
    ...
    pub async fn request<T: DeserializeOwned>(&self, method: &str, params: Value) -> Result<T, ApiError> {
        ...
        let result = tokio::time::timeout(REQUEST_TIMEOUT, rx)
            .await
            .map_err(|_| ApiError::Internal(format!("OpenClaw request '{method}' timed out")))?
            .map_err(|_| ApiError::Internal(format!("OpenClaw request '{method}' cancelled")))??;

        serde_json::from_value(result)
            .map_err(|e| ApiError::Internal(format!("Failed to parse OpenClaw response for '{method}': {e}")))
    }
    ...
    pub async fn close(&self) {
        ...
        for (_, tx) in pending.drain() {
            let _ = tx.send(Err(ApiError::Internal("Connection closed".into())));
        }
    }
    ...
    async fn run_reader(self: Arc<Self>, mut stream: WsStream) {
        ...
        let mut pending = self.pending.lock().await;
        for (_, tx) in pending.drain() {
            let _ = tx.send(Err(ApiError::Internal("OpenClaw connection closed".into())));
        }
    }
            ...
            IncomingFrame::Res(res) => {
                ...
                if let Some(tx) = pending.remove(&res.id) {
                    if res.ok {
                        ...
                        let _ = tx.send(Err(ApiError::Internal(msg)));
                    }
                }
            }
    ...
    async fn ws_send_frame(&self, frame: &RequestFrame) -> Result<(), ApiError> {
        let text = serde_json::to_string(frame)
            .map_err(|e| ApiError::Internal(format!("Failed to serialize request frame: {e}")))?;

        let mut guard = self.ws_sink.lock().await;
        let sink = guard
            .as_mut()
            .ok_or_else(|| ApiError::Internal("OpenClaw WebSocket not connected".into()))?;

        sink.send(Message::Text(text.into())).await.map_err(|e| {
            error!(error = %e, "Failed to send OpenClaw WebSocket message");
            ApiError::Internal(format!("OpenClaw WebSocket send failed: {e}"))
        })
    }
}
