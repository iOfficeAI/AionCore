...
pub fn load_or_create_identity(custom_path: Option<&Path>) -> Result<DeviceIdentity, ApiError> {
...
fn load_identity(path: &Path) -> Result<DeviceIdentity, ApiError> {
    let content =
        fs::read_to_string(path).map_err(|e| ApiError::Internal(format!("Failed to read device identity: {e}")))?;

    let stored: StoredIdentity = serde_json::from_str(&content)
        .map_err(|e| ApiError::Internal(format!("Failed to parse device identity: {e}")))?;
...
fn save_identity(path: &Path, identity: &DeviceIdentity) -> Result<(), ApiError> {
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)
            .map_err(|e| ApiError::Internal(format!("Failed to create identity directory: {e}")))?;
    }
    ...
    let json = serde_json::to_string_pretty(&stored)
        .map_err(|e| ApiError::Internal(format!("Failed to serialize device identity: {e}")))?;

    fs::write(path, format!("{json}\n"))
        .map_err(|e| ApiError::Internal(format!("Failed to write device identity: {e}")))?;
...
fn pem_to_signing_key(pem: &str) -> Result<SigningKey, ApiError> {
    ...
    if der.len() == ED25519_PKCS8_PREFIX.len() + 32 && der[..ED25519_PKCS8_PREFIX.len()] == ED25519_PKCS8_PREFIX {
        let raw: [u8; 32] = der[ED25519_PKCS8_PREFIX.len()..]
            .try_into()
            .map_err(|_| ApiError::Internal("Invalid Ed25519 private key length".into()))?;
        return Ok(SigningKey::from_bytes(&raw));
    }

    Err(ApiError::Internal("Unrecognized Ed25519 PKCS8 format".into()))
}
...
fn pem_decode(pem: &str, label: &str) -> Result<Vec<u8>, ApiError> {
    use base64::engine::general_purpose::STANDARD;
    ...
    if !pem.contains(&begin) || !pem.contains(&end) {
        return Err(ApiError::Internal(format!("Invalid PEM: missing {label} markers")));
    }

    STANDARD
        .decode(b64)
        .map_err(|e| ApiError::Internal(format!("Failed to decode PEM base64: {e}")))
}
