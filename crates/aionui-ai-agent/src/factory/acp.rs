...
use aionui_common::{ApiError, CommandSpec};
...
pub(super) async fn build(
    ...
    let mut config: AcpBuildExtra = serde_json::from_value(options.extra)
        .map_err(|e| ApiError::BadRequest(format!("Invalid ACP build options: {e}")))?;
    ...
    .ok_or_else(|| ApiError::BadRequest("ACP agent requires either agent_id or backend in extra".into()))?;
...
async fn resolve_agent_command_spec(
    ...
    let command = meta
        ...
        .ok_or_else(|| ApiError::BadRequest(format!("Agent '{}' has no spawn command configured", meta.name)))?;
    ...
    let resolved = ensure_runtime_command_with_reporter(command, Some(reporter.as_ref()))
        .await
        .map_err(|error| ApiError::BadRequest(format!("Agent '{}' CLI unavailable: {error}", meta.name)))?;
