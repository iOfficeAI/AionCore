...
impl FactoryContext {
    pub async fn resolve(deps: &AgentFactoryDeps, options: &BuildTaskOptions) -> Result<Self, ApiError> {
        ...
        let (workspace, is_custom_workspace) = if options.workspace.is_empty() {
            ...
            std::fs::create_dir_all(&dir)
                .map_err(|e| ApiError::Internal(format!("Failed to create temp workspace: {e}")))?;
