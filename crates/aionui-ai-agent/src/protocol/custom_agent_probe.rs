...
async fn run_handshake(proc: &CliAgentProcess) -> Result<(), String> {
    ...
    tokio::select! {
        ...
        res = connect => {
            ...
            // Drop `protocol` so its shutdown oneshot fires before the
            // caller's process-group SIGKILL. The grace window in
            // `proc.kill()` lets a well-behaved CLI exit on its own.
            drop(protocol);
            Ok(())
        }
