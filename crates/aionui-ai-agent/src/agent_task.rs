...
pub trait IMockAgent: IAgentTask {
    ...
    async fn mode(&self) -> Result<aionui_api_types::AgentModeResponse, ApiError> {
        Ok(aionui_api_types::AgentModeResponse {
            mode: "default".into(),
            initialized: false,
        })
    }
    async fn set_mode(&self, _mode: &str) -> Result<(), ApiError> {
        Err(ApiError::BadRequest(
            "Mode switching is not supported for this mock".into(),
        ))
    }
    async fn get_model(&self) -> Result<GetModelInfoResponse, ApiError> {
        Ok(GetModelInfoResponse { model_info: None })
    }
    async fn set_model(&self, _model_id: &str) -> Result<(), ApiError> {
        Err(ApiError::BadRequest(
            "Model switching is not supported for this mock".into(),
        ))
    }
    async fn get_usage(&self) -> Result<Option<serde_json::Value>, ApiError> {
        Ok(None)
    }
    async fn get_slash_commands(&self) -> Result<Vec<SlashCommandItem>, ApiError> {
        Ok(Vec::new())
    }
    async fn handle_side_question(&self, _req: SideQuestionRequest) -> Result<SideQuestionResponse, ApiError> {
        Ok(SideQuestionResponse {
            status: "unsupported".into(),
            answer: None,
        })
    }
    async fn get_openclaw_runtime(&self) -> Result<serde_json::Value, ApiError> {
...
impl AgentInstance {
    ...
    /// Terminate the agent process.
    pub fn kill(&self, reason: Option<AgentKillReason>) -> Result<(), ApiError> {
    ...
    pub async fn get_mode(&self) -> Result<aionui_api_types::AgentModeResponse, ApiError> {
        match self {
    ...
    pub async fn set_mode(&self, mode: &str) -> Result<(), ApiError> {
        match self {
            ...
            Self::OpenClaw(_) | Self::Nanobot(_) | Self::Remote(_) => Err(ApiError::BadRequest(
    ...
    pub async fn get_model(&self) -> Result<GetModelInfoResponse, ApiError> {
        match self {
            Self::Acp(m) => {
    ...
    pub async fn set_model(&self, model_id: &str) -> Result<(), ApiError> {
        if model_id.trim().is_empty() {
            return Err(ApiError::BadRequest("model_id must not be empty".into()));
        }
        match self {
            Self::Acp(m) => m.set_model(model_id).await,
            Self::Aionrs(_) | Self::OpenClaw(_) | Self::Nanobot(_) | Self::Remote(_) => Err(ApiError::BadRequest(
    ...
    pub async fn get_usage(&self) -> Result<Option<serde_json::Value>, ApiError> {
        match self {
            Self::Acp(m) => {
                let Some(usage) = m.usage().await else { return Ok(None) };
                let mut value = serde_json::to_value(usage)
                    .map_err(|e| ApiError::Internal(format!("Failed to serialize usage: {e}")))?;
    ...
    pub async fn handle_side_question(&self, req: SideQuestionRequest) -> Result<SideQuestionResponse, ApiError> {
    ...
    pub async fn get_openclaw_runtime(&self) -> Result<serde_json::Value, ApiError> {
        match self {
