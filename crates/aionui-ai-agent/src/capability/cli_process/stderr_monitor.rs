...
pub(super) fn force_kill(pid: u32, process_group_id: Option<u32>) -> Result<(), ApiError> {
    #[cfg(unix)]
    {
        use std::io;

        fn kill_pid(target_pid: u32) -> Result<(), ApiError> {
    ...
    #[cfg(windows)]
    {
        ...
        match result {
            ...
            Err(e) => {
                ...
                Err(ApiError::Internal(format!("Failed to kill process {pid}: {e}")))
            }
        }
    }
